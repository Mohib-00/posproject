                    ...
                    <div class="card">
                      ...
                      <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            <a class="addcategory" >Add category</a>
                        </div>
                    </div>
                    
                    <h1 class="mx-3 list">category List</h1>
                            ...
                            <tbody>
                                ...
                                @foreach($categorys as $category)
                                        <tr class="user-row" id="category-{{ $category->id }}">
                                                ...
                                                <td id="name">{{$category->brand_name}}</td>  
                                                <td id="heading">{{$category->category_name}}</td>  
                                                <td id="user_id">{{ $category->user->name }}</td>
                                                <td id="slug">{{$category->created_at}}</td>
                                                <td>
                                                    ...
                                                    <a data-category-id="{{ $category->id }}" class="btn btn-link btn-danger delcategory mt-2">
            ...
            <div class="modal-content">
                ...
                <form id="categoryeditform">
                    ...
                    <div class="row mt-5">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                <label for="mobile_edit">Category Name</label>
                                <input type="text" id="mobile_edit" name="category_name" class="form-control">
                            </div>
                        </div>
            ...
            removeLoader();
               if (response.success) {
                removeLoader();
                   ...
                   $('#categoryeditform #name_edit').val(response.category.brand_name);
                   $('#categoryeditform #mobile_edit').val(response.category.category_name);
               ...
               if (response.success) {
                removeLoader();
                   Swal.fire({
                       ...
                       category.find('td:nth-child(2)').text(response.category.id);
                       category.find('td:nth-child(3)').text(response.category.brand_name);
                       category.find('td:nth-child(4)').text(response.category.category_name);
                       category.find('td:nth-child(5)').text(response.user_name); 
                       category.find('td:nth-child(6)').text(response.category.created_at);
                   });
               } else {
                   Swal.fire({
                       icon: 'error',
